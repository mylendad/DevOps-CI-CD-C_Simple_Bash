...
flags parser(int argc, char **argv) {
  ...
  errno = 22;
  int arguments;
  while ((arguments = getopt(argc, argv, "e:ivclnhsf:o")) != -1) {
    switch (arguments) {
      case 'e':
        argument.e = optind;
...
void add_pattern(flags *argument, char *pattern) {
  ...
  if (len_pattern < len_pattern + argument->len) {
    argument->reg_pattern =
        realloc(argument->reg_pattern,
                (len_pattern + argument->len + 4) * sizeof(char));
  }
  if (argument->len > 0) {
  ...
  argument->len =
      argument->len +
      (sprintf(argument->reg_pattern + argument->len, "(%s)", pattern));
}
...
void flag_o(flags *argument, regex_t *reg, char *line, char *file_name,
            int line_counter) {
  regmatch_t match = {0};
  int shift = 0;
  int result;
  int count = 0;
  while ((result = regexec(reg, line + shift, 1, &match, 0)) == 0) {
    if (count > 0) {
      if (argument->h == 0) {
        printf("%s:", file_name);
      }
...
void comparator(flags *argument, char *file_name, regex_t *reg, int argc,
  ...
  while (fgets(line, sizeof(line), f) != NULL) {
    ...
    if (((result == 0 && argument->v == 0) ||
         (result != 0 && argument->v == 1)) &&
        argument->c == 0 && argument->l == 0) {
      ...
      if (result == 0 && argument->o == 1 && argument->c == 0) {
        flag_o(argument, reg, line, file_name, line_counter);
      } else {
  ...
  if (argument->c == 1) {
    char *name_file = argv[count_filename];
    if (argv[4] != NULL) {
      if (argument->h == 0 && (argument->o == 0 || argument->c == 1)) {
        printf("%s:", name_file);
      }
    }
    printf("%d\n", count_reg);
...
void output(flags *argument, int argc, char **argv) {
  ...
  int count_filename;
  for (count_filename = optind; count_filename < argc; count_filename++) {
    if (argv[4] != NULL && argument->h == 0 && argument->l == 0 &&
        argument->s == 0 && argument->o == 0 && argument->f == 0 &&
        argument->e == 0 && argument->v == 0 && argument->i == 0 &&
        argument->n == 0 && strstr("-", argv[1]) != NULL) {
      if (strstr("-", argv[4]) == NULL) {
        printf("%s:", argv[count_filename]);
      }
    }
    comparator(argument, argv[count_filename], &reg, argc, argv,
               count_filename);
  }
  regfree(&reg);
}